...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header id="main-header" className="w-full bg-slate-950/90 border-b border-slate-800 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-3">
        {/* Brand logo */}
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => setActiveTab('solo')}>
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-emerald-500 to-teal-400 flex items-center justify-center shadow-lg shadow-emerald-500/20">
            <Dices className="w-6 h-6 text-slate-950 stroke-[2.5]" />
          </div>
          <div>
            <h1 className="text-xl font-black tracking-wider text-slate-100 uppercase flex items-center gap-1.5">
              YAHTZEE <span className="text-emerald-400 font-extrabold">WILD</span>
            </h1>
            <p className="text-[10px] text-slate-400 font-medium">Real-Time Multiplayer & Leaderboard</p>
          </div>
        </div>
        ...
        {/* Right Controls: Sound Toggle & OAuth/Profile */}
