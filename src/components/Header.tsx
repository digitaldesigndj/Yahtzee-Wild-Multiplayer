...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header id="main-header" className="w-full bg-slate-950/95 border-b border-slate-800/80 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2.5 flex flex-col lg:flex-row items-center justify-between gap-2.5 lg:gap-4">
        ...
        <div className="w-full lg:w-auto flex items-center justify-between gap-2 shrink-0">
          ...
          <div className="flex items-center gap-2.5 cursor-pointer shrink-0" onClick={() => setActiveTab('solo')}>
            <img
              src="/src/assets/images/app_icon_1785785319120.jpg"
              alt="Yahtzee Wild Icon"
              className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl shadow-lg shadow-emerald-500/20 object-cover border border-emerald-500/30"
          ...
          <div className="flex lg:hidden items-center gap-1.5 shrink-0">
            ...
            {user ? (
              ...
              <button
                type="button"
                onClick={onOpenAuthModal}
                className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-emerald-500 text-slate-950 font-black text-xs cursor-pointer"
              >
        ...
        {/* Navigation Tabs Bar */}
        <nav className="w-full lg:w-auto max-w-full overflow-x-auto no-scrollbar flex items-center justify-between sm:justify-center gap-1 bg-slate-900 p-1 rounded-xl border border-slate-800/80 shrink-0">
          <button
            type="button"
            onClick={() => { sounds.playClickSound(); setActiveTab('solo'); }}
            className={`flex-1 sm:flex-none flex items-center justify-center gap-1 sm:gap-1.5 px-2.5 sm:px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all shrink-0 whitespace-nowrap cursor-pointer ${
              activeTab === 'solo'
          ...
          <button
            ...
            className={`flex-1 sm:flex-none flex items-center justify-center gap-1 sm:gap-1.5 px-2.5 sm:px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all relative shrink-0 whitespace-nowrap cursor-pointer ${
